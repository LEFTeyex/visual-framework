...
from utils.typeslib import _str_or_None, _pkt_or_None, _path, _dataset, \
...
class SetSavePathMixin(object):
    ...
    1. Set save path (then get them in dict). --- all self.*
    """
...
class TensorboardWriterMixin(object):
    ...
    def set_tensorboard_writer(self, path):
...
class SaveCheckPointMixin(object):
    ...
    def save_checkpoint(self, fitness):
        ...
        if self.epoch + 1 == self.epochs:
            ...
            checkpoint = self.get_checkpoint()
            torch.save(checkpoint, self.path_dict['last'])
...
class LoadAllCheckPointMixin(object):
    ...
    def set_param_groups(self, param_kind_tuple: _pkt_or_None = None):
        ...
        for name, kind, param_dict in param_kind_tuple:
            ...
            param_dict['name'] = f'{kind.__name__}.{name}'  # to record in tensorboard for lr corresponding to params
...
class DataLoaderMixin(object):
    r"""
    Need self.writer, self.workers, self.batch_size, self.pin_memory, self.visual_image.
    The function in the Mixin below.
    1. Set dataloader. --- all self.*
    """
    ...
    def set_dataloader(self, dataset_instance: _dataset, shuffle: bool = False):
        r"""
        Set  dataloader.
        Args:
            dataset_instance: _dataset = dataset class
    ...
    def train_one_epoch(self):
        ...
        with tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader),
            ...
            for index, (images, labels, _, _) in pbar:
                ...
                loss_all_mean = loss_to_mean(index, loss_all_mean, loss_all)
...
class TrainClassifyMixin(object):
    def __init__(self):
        self.inc = None
        self.cuda = None
        self.model = None
    ...
    def train_one_epoch(self):
        ...
        with tqdm(enumerate(self.train_dataloader), total=len(self.train_dataloader),
            ...
            for index, (images, labels, _, _) in pbar:
                ...
                self._show_loss_in_pbar_training(loss_mean, pbar)
...
def loss_to_mean(num_loop, loss_mean, loss):
    r"""The number of loop"""
    loss_mean = (loss_mean * num_loop + loss) / (num_loop + 1)
    return loss_mean
...
class ResultsDealDetectMixin(object):
    ...
    def save_all_results(self):
        ...
        for k, v in self.results.items():
            to_save.append((v, self.path_dict[k]))
